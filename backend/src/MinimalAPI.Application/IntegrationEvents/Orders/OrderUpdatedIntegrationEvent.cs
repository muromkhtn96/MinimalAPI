...
/// <summary>
/// Integration event phát lên Kafka khi đơn hàng được cập nhật thành công (sau khi commit DB).
/// Topic: minimalapi.orders (khai báo trong appsettings — Kafka:Topics) — chung topic với OrderCreated,
...
public sealed record OrderUpdatedIntegrationEvent : IntegrationEvent
{
    ...
    // Cùng OrderId → cùng partition với OrderCreated (chung topic minimalapi.orders)
    // → mọi event của một đơn hàng giữ đúng thứ tự created → updated → ...
